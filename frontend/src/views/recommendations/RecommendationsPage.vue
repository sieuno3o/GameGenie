                ...
                <v-row>
                    <v-col v-for="game in recommendedGames" :key="game.appid" cols="12" md="4">
                        <v-card>
                            ...
                            <v-card-actions>
                                <v-btn :href="game.store_url" target="_blank">상점 페이지로 가기</v-btn>
                            </v-card-actions>
                        </v-card>
                    </v-col>
                </v-row>
...
export default {
    ...
    created() {
        const userInputFromQuery = this.$route.query.user_input;
        if (userInputFromQuery) {
            this.userInput = userInputFromQuery;
            this.sendQuery();
        }
    },
    methods: {
